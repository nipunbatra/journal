...
K=3
num_trials=25
min_common_features=20
num_homes,  = sys.argv[1:]
num_homes = int(num_homes)
...
all_homes_df = out_overall[test_region]
all_homes_df = all_homes_df[(all_homes_df.full_agg_available == 1) & (all_homes_df.md_available == 1)]
all_homes_df = all_homes_df.ix[all_homes_df[['%s_%d' %(appliance,month) for month in range(start,stop)]].dropna().index]



np.random.seed(0)
for count, test_home in enumerate(all_homes_df.index):
    ...
    for trial in range(num_trials):
        ...
        print count, len(test_df), num_homes, trial
        ...
        for month_compute in range(month_start, month_end):
            ...
            co = defaultdict(int)
            store_path = '../../../output/unified_num_homes_%d/%d/%d/%d_%s_%s_%s_%s_%d_%d_%d.pkl' % (
                num_homes, min_common_features,num_trials,
                trial, train_region,
                test_region,
                transform,
                appliance,
                ...
                test_home, K)
            #print store_path
            store_base_path = '../../../output/unified_num_homes_%d/%d/%d' %(num_homes, min_common_features, num_trials)
