...
for transform in transformations:
    ...
    for appliance in APPLIANCES:
        ...
        for trial in range(0, num_trials, 2):
            if appliance=="hvac":
                start, stop=5, 11
            else:
                start, stop=1, 13
            ...
            out[transform][appliance][trial] = {}
            for month in range(start,stop):
                print appliance, month, transform
                out[transform][appliance][trial][month] = {}
                for test_home in test_df_appliance.index[:]:
                    try:
                        pred =  pickle.load(open('../../../output/output/unified_num_homes_%d/%d_%s_%s_%s_%s_%d_%d_%d.pkl' %(num_trials,trial,train_region,
                                                                                test_region,
                                                                                transform,
                                                                                appliance,
                                                                                month,
                        ...
                        out[transform][appliance][trial][month][test_home] = percentage_accuracy
                    except Exception, e:
                        print e
